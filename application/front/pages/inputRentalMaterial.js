...
import { INPUT_RENTAL_MATERIAL_REQUEST } from '../reducers/material';
...
const InputRentalMaterial = () => {
  const [file, setFile] = useState('');
  const [contract, onChangeContract] = useInput('');
  const [rentalCompany, onChangeRentalCompany] = useInput('');
  const [rental, onChangeOrder] = useInput('');
  ...
  const onSubmitForm = useCallback((e) => {
    ...
    dispatch({
      type: INPUT_RENTAL_MATERIAL_REQUEST,
      data: {
        fileName,
        contract,
        rentalCompany,
  ...
  }, [date, contract, rentalCompany, rental, file]);
    ...
    <AppLayout title="자재정보입력(임대업체)">
      <Wrapper>
        ...
        <form onSubmit={onSubmitForm}>
          ...
          <MaterialInput type="text" value={rentalCompany} placeholder="조달업체명을 입력해주세요" onChange={onChangeRentalCompany} required />
          <InputWrapper>
            <AmountInput type="number" value={rental} placeholder="조달수량을 입력해 주세요" onChange={onChangeOrder} />
...
export default InputRentalMaterial;
